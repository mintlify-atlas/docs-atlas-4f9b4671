...
<ResponseField name="data" type="object">
  ...
  <ResponseField name="data.idItem" type="integer">
    Auto-generated numeric ID for the new item. Use this value in subsequent calls to assign the item to a role.
  </ResponseField>
...
<Note>
  After creating an item, it is not visible to any user yet. Use `POST /api/v1/sidebar/:idItem/role/:idRole` to assign the new item to one or more roles. Users with those roles will then see it in their sidebar at next login or when calling `GET /api/v1/sidebar`.
</Note>
