---
title: "DELETE /api/v1/permission/unassign — Remove a Permission from a Role"
sidebarTitle: "Unassign Permission"
description: "Removes the link between a permission and a role, revoking access to that route for users with that role. Requires JWT and the DELETE /api/v1/permission/unassign permission."
---
...
Returned when the unassignment completes. If the combination did not exist, the operation succeeds silently (no error is raised).
...
| `404 Not Found` | Reserved for service-level checks; not currently raised by this endpoint (non-existent pairs are silently ignored). |
...
| `data:changed` | All clients | `permissions:unassign` | Broadcast (no payload data) |
