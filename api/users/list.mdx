---
title: "GET /api/v1/users — List All Users in the System"
sidebarTitle: "List Users"
description: "Returns a list of all user accounts with role information. Requires a valid JWT token and the GET /api/v1/users permission on the caller's role."
---

The `GET /api/v1/users` endpoint returns a collection of all user records stored in the system. Each user object includes their role name and role description, sourced from a `LEFT JOIN` with the `roles` table. The `password` field is never included in results.
